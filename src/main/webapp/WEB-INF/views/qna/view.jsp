...
<style>
@import
	url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500&display=swap');
...
function sendcomments() {
	...
	/* $.ajax({
		...
		success: fybction(html) {
			$("#message").html(html);
		},
		error: function(xhr) {
			$("message").html("Error Code : "+xhr.status);
		}
	}); */
...
<body>
	...
	<div class="subbox">
		...
		<p class="subboxsub text-center">Do you have anything that you are
			curious about?</p>
	</div>
	...
	<div class="contentcontainer padding-top50">
		...
		<div class="margintop10">
			...
			<div class="boxcontent">${item.writing}</div>
		</div>
		<div class="flex flex-justify-between marginbottom50">
			<div class="buttonupdatefont flex text-center ">
				<%-- <div class = "buttonboxview marginright20">
						<a href = "/request/update/${item.number}" class = "atext paddingtop10">수정</a>
					</div> --%>
				<div class="buttonboxview text-center marginright20">
					<a href="/qna/update/${item.number}"
						class="buttonupdatefont paddingtop10 atextone"
						onclick="check_name_Sync()"> 수정 </a>
				</div>
				<div class="buttonboxview text-center">
					<a href="/qna/delete/${item.number}"
						class="buttonupdatefont paddingtop10 atextone"
						onclick="check_name_Sync()"> 삭제 </a>
				</div>
			</div>
	...
	<div class="contentcontainer">
		...
		<div class="margintop4">
			<textarea class="textareabox tiplytext" id="replytext" rows="3"
				placeholder="답글을 입력하세요"></textarea>
			<button class="tiplybox buttonupdatefont margintop16" type="button"
				onclick="sendcomments();">등록</button>
		</div>
	</div>
	<div class="replymain">
